...
public class NumberDecimalMatcher implements TokenMatcher {
    // TODO : negative numbers
    // TODO : allow spaces between number, eg "12 000"

    private static final TokenRegexMatcher MATCHER_SIMPLE = TokenRegexMatcherBuilder.start().capture("\\d+").or(Separator.POINT, Separator.COMMA)
            .capture("\\d+").build();
    ...
    public PatternMatched match(Token token, TokenProvider tokenFis) throws IOException {
        ...
        if (matchInt != null) {
            return new PatternMatched(EquivalenceClass.DECIMAL,
                    matchInt.getExtractedValue(0) + Separator.COMMA.getOfficialCharString() + matchInt.getExtractedValue(1),
                    matchInt.getLastMatchedToken());
        }
        return null;
    }
}
