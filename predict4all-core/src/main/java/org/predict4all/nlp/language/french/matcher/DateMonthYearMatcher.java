...
public class DateMonthYearMatcher implements TokenMatcher {

    private static final TokenRegexMatcher MATCHER_SIMPLE = TokenRegexMatcherBuilder.start().capture(FrenchLanguageUtils.getMonthOrRegex())
            .then(Separator.SPACE).capture("\\d{2,4}").build();
    ...
    public PatternMatched match(Token token, TokenProvider tokenFis) throws IOException {
        ...
        if (match != null) {
            int yearValue = Integer.parseInt(match.getExtractedValue(1));
            return new PatternMatched(EquivalenceClass.DATE_MONTH_YEAR,
                    FrenchLanguageUtils.MONTHS.get(match.getExtractedValue(0).toLowerCase()) + Separator.SPACE.getOfficialChar()
